...
function VendorCard({ vendor }: { vendor: Vendor }) {
  return (
    <div className="rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-200/60 dark:border-white/8 overflow-hidden flex flex-col">
      <Link
        to="/vendors/$id"
        params={{ id: vendor.id }}
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="space-y-1">
          <Link
            to="/vendors/$id"
            params={{ id: vendor.id }}
            className="text-base font-bold text-neutral-900 dark:text-white hover:underline"
          >
            {vendor.name}
          </Link>
        ...
        <p className="text-sm text-neutral-500 dark:text-neutral-400 leading-relaxed mt-5 mb-5 line-clamp-3">
          {vendor.description}
        </p>
        ...
        <div className="mt-auto pt-5 border-t border-neutral-100 dark:border-white/[0.04] flex gap-5">
          <Link
            to="/vendors/$id"
            params={{ id: vendor.id }}
            className="inline-flex items-center justify-center gap-1.5 rounded-full border border-neutral-200/60 dark:border-white/[0.06] px-3.5 py-1.5 text-sm font-medium text-neutral-700 dark:text-neutral-200 hover:bg-neutral-50 dark:hover:bg-white/[0.04] transition-colors flex-1"
          >
          ...
          <a
            ...
            className="inline-flex items-center justify-center gap-1.5 rounded-full bg-neutral-900 dark:bg-white px-3.5 py-1.5 text-sm font-medium text-white dark:text-neutral-900 hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-colors flex-1"
          >
...
export function VendorGrid({ data }: VendorGridProps) {
  if (data.length === 0) {
    return (
      <div className="rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-200/60 dark:border-white/8 py-20 text-center">
